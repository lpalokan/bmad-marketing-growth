...
3. Load company context: `positioning.md`, `icp.md`, `brand-voice.md`. Read whichever of those are present and note what is absent — a missing file is never a hard stop. Only if the bundle itself does not exist anywhere in the project, say so once, suggest `/company-context-bootstrap`, and ask the user whether to continue without it.
